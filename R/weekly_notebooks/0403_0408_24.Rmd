...
Might not do random effect of study in gamlss (although Margaret said she was able to get this and the code is on github, I have not sorted out the specific erros on it) ...
For ABCD, might be able to use timepoint 1 as a predictor for timepoint 2, instead of a longitudinal random effects model where the subject is a random effect
Per discussion with Aaron, I should focus on writing my proposal. Any prelim figures?
Also need to apply for Neurohackademy
Also need to think about abstracts for conferences (early May).

For ARCUS/SLIP cohort
- Need a SQL script to pull specific people and write and R script for my specific cohort, which will then create a new project.

For ABCD -- Lena mentioned some issues with the diagnosis encoding (there is a pdf she sent on Slack)
This week will try briefly to get a linear model as first pass working on ABCD.
...
-For trajectory differences ... further down the line.
  ...
  - might want to use T1 as a predictor for T2 (as explained above)
...
**Combine the VPM & LPM
```{r}
df <- read.csv("/Users/ekafadar/Documents/Grad_School/BGDLab/SLIP_data/SLIP_GA_SsQC_65.csv") %>%rename(GAbins_recode = preterm) %>% arrange(logAge)
...
df_term <- df %>% filter(GAbins_recode == "Term")
gc_term <- lapply(vars, 
                 function(x) {growthchart_model(
                   p = x, 
                   df = df_term)}) %>% set_names(vars)
...
plots_term <- lapply(seq_along(vars), function(i){growthChart_plot(p = vars[i], df = df_term, centileCurves = gc_term[[i]]$centileCurves, title = "")})

plotsGA <- lapply(seq_along(vars), function(i){growthChart_plot(p = vars[i], df = df, centileCurves = gc_allGA[[i]]$centileCurves, title = "", by.GAbins = TRUE)})
...
write.csv(abcd_wide, file = "/Users/ekafadar/Documents/Grad_School/BGDLab/ABCD_data/abcd_combined_wide_032524.csv")
write.csv(abcd_long, file = "/Users/ekafadar/Documents/Grad_School/BGDLab/ABCD_data/abcd_combined_long_032524.csv")
...
abcd_long <- read.csv("/Users/ekafadar/Documents/Grad_School/BGDLab/ABCD_data/abcd_combined_long_032524.csv")
abcd_wide <- abcd_wide[,-1] #remove the row names column
abcd_long <- abcd_long[,-1] #remove the row names column
```
...
hist(abcd_wide[!is.na(abcd_wide$smri_vol_scs_intracranialv_baseline_year_1_arm_1) & abcd_wide$gestAge != "0" ,]$gestAge, main = "Gestational Age for Baseline MRI Scans", xlab = "Gestational Age")
...
hist(abcd_wide[!is.na(abcd_wide$smri_vol_scs_intracranialv_baseline_year_1_arm_1) & !is.na(abcd_wide$smri_vol_scs_intracranialv_2_year_follow_up_y_arm_1) & abcd_wide$gestAge != "0" ,]$gestAge, main = "Gestational Age for 2nd TimePoint MRI Scans", xlab = "Gestational Age")
...
```
##Linear models
How do you visualize predictions when holding one effect the same?
Re-arrange levels of preterm so that baseline is "Term". And then try to figure out significance.
...
mod1 <- lmer(smri_vol_scs_intracranialv ~ 1 + PCW_at_event + sex + site_id_l + (1 | src_subject_id), data = df)

 #generating an evenly spaced grid from data points to visualize a model
newdata <- modelr::data_grid(df, PCW_at_event, sex, site_id_l)
newdata$predictions <- predict(mod1, newdata = newdata, re.form=~0)
#plot
ggplot(newdata,aes(x= PCW_at_event, y = predictions, color = as.factor(sex)))+
  geom_point() +
  geom_line(aes(y = predictions))

##Model with Preterm status
df_all <- abcd_long %>% filter(!is.na(preterm) & sex %in% c("M", "F") & !is.na(smri_vol_scs_intracranialv)) %>% mutate(logAge = log10(PCW_at_event))
df_all$preterm <- as.factor(df_all$preterm)
df_all$site_id_l <- as.factor(df_all$site_id_l)

mod2 <- lmer(smri_vol_scs_intracranialv ~ 1 + PCW_at_event + sex + preterm + site_id_l + (1 | src_subject_id), data = df_all)

summary(mod2)
anova(mod2)

newdata_all<- modelr::data_grid(df_all, PCW_at_event, sex, preterm, site_id_l)
newdata_all$predictions <- predict(mod2, newdata = newdata_all, re.form=~0)


ggplot(newdata_all,aes(x= PCW_at_event, y = predictions, color = as.factor(preterm)))+
  geom_point() +
  geom_line(aes(y = predictions))

#keep siteID same
newdata_all_F<- modelr::data_grid(df_all, PCW_at_event, preterm)
newdata_all_M<- modelr::data_grid(df_all, PCW_at_event, preterm)
newdata_all_F$site_id_l <- rep("site01", dim(newdata_all_F)[1])
newdata_all_M$site_id_l <- rep("site01", dim(newdata_all_M)[1])
newdata_all_F$sex <- rep("F", dim(newdata_all_F)[1])
newdata_all_M$sex <- rep("M", dim(newdata_all_M)[1])
newdata_all_F$predictions <- predict(mod2, newdata = newdata_all_F, re.form=~0)
newdata_all_M$predictions <- predict(mod2, newdata = newdata_all_M, re.form=~0)
newdata_all <- bind_rows(newdata_all_F, newdata_all_M)

ggplot(newdata_all_F,aes(x= PCW_at_event, y = predictions, color = as.factor(preterm)))+
  geom_point() +
  geom_line(aes(y = predictions))
ggplot(newdata_all_M,aes(x= PCW_at_event, y = predictions, color = as.factor(preterm)))+
  geom_point() +
  geom_line(aes(y = predictions))
