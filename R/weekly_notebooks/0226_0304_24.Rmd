...
WANT TO DO NEXT
>> did this below, but only within SLIP 03/01
...
#Aaron mtg 02/26
We mostly discussed specific aims. Comments are on the specific aims Draft 4 google docs.
***
...
gc_MPR <- lapply(paste0(vars,"_TRUE"), function(x) {growthchart_model(x, df = wide_data)})
gc_nonMPR <- lapply(paste0(vars,"_FALSE"), function(x) {growthchart_model(x, df = wide_data)})
...
lbcc$study_recode <- paste(lbcc$study, lbcc$site, sep = "_")
...
slip_GASsQc$study_recode <- rep("SLIP", length(slip_GASsQc$study))
```
...
lbcc_slip$subject_study <- paste0(lbcc_slip$participant, lbcc_slip$study)
