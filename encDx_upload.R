...
rowcount_query <- 'SELECT count(*) FROM lab.proc_encDx_EK'
...
filename <- glue("/mnt/arcus/lab/users/kafadare/GAB_cohort/excluded_procIDs/proc_ord_ids_exclude_encDx_part{n}.rds")
print(paste0("Reading file: ", filename))

#Upload phecodes_excluded to SQL arcus
procedures_excluded <- readRDS(filename)
bq_auth()
proj_id <- if (exists('proj_id')) proj_id else bq_projects()[1]
